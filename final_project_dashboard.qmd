...
# Sub-Saharan Africa is the region with most babies per women.
babies_line_mean = (
    px.line(
        combined_mean,
        x="year",
...
# only Sub-Saharan Africa is the region with mose babies per women.
```
...
```{python}
# Get the highest, lowest and average nr. babies per women
...
# Hghest birth rate in the 100 year perios is Yemen with 8.86 babies per woman in 1985!
# Lowest birth rate in the 100 year period is South Korea with 0.81 babies per women in 2020!
```
...
income_babies_scatter = (
    px.scatter(
        ...
        # title="Low average daily income (USD) is associated with increased number of babies per women",
        # title="Number of babies per women decreases as wages increase"
    )
    ...
    .add_annotation(
        text="No. of babies per women",  # Global y-axis label
        x=-0.1,
...
::: {.valuebox title="Highest birth rate" color="success" icon="arrow-up"}
`{python} max_babies_country` (`{python} max_babies_year`) with
`{python} max_babies_count` babies per woman
:::

::: {.valuebox title="Lowest birth rate" color="#ffeda0" icon="arrow-down"}
...
The number of babies per woman was high (5-8) in most of the world in 1920, expept for North America, Europe and Australia. Over the course of 100 years, this number decreased in almost all countries, with central Africa being the region with the highest birth rate.
:::

::: {.card}
### Sub-Saharan Africa has double the birth rate as America {width="50%"}
...
::: {.card}
### Lowest in come in two regions, including Sub-Saharan Africa {width="50%"}
...
::: {.card}
### High daily income (USD) is associated with low birth rate {width="50%"}

```{python}
income_babies_scatter
```
Data points were plotted regardless of which year they refer to (i.e. they are not in chronological order).
The plot shows ther there is a general trend where higher income is associated with a low number of babies per woman, regardless of historical time. Exceptinos to this trend include Qatar, Kuwait, UAE, Brunai, and Equatorial Guinea.
:::

::: {.card}
### Income and birth rate move in opposite directions {width="50%"}
...
- Babies per women (fertility rate):
